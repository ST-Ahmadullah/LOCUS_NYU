...
public class EditandDelete {
    ...
    public void createusermanagement() throws Throwable {
        ...
        wait7.until(ExpectedConditions.elementToBeClickable(By.xpath("//select[@id='academic-term-select']/option[text()='new terms']"))).click();
        ...
        WebDriverWait menu = new WebDriverWait(driver, Duration.ofSeconds(10));  // Updated line
        menu.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//button[@id='menuButton']"))).click();
        Thread.sleep(2000);

        WebDriverWait myclasses = new WebDriverWait(driver, Duration.ofSeconds(20));  // Updated line
        myclasses.until(ExpectedConditions.elementToBeClickable(By.xpath("(//a[@class='router-link-active router-link-exact-active active'])[1]"))).click();
        Thread.sleep(2000);
